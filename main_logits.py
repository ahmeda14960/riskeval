...
filtered_df = filtered_df[
    filtered_df["decision_question_id"].isin(sampled_question_ids)
]
...
for model_name in tqdm(
    ...
    answer_labels = [
        1,
        0,
    ]  # Assign labels to the answer choices
    ...
    for index, row in tqdm(
        ...
        input_encoding = {
            k: v.to(device) for k, v in input_encoding.items()
        ...
        cost = costs_df.loc[
            costs_df["decision_question_id"] == decision_question_id,
            "cost",
        ].values[0]
    ...
    for group in tqdm(
        demographic_groups,
        desc="Converting logits to numpy arrays",
    ):
    ...
    for group, yes_logits in tqdm(
        group_yes_logits.items(),
        desc="Computing discrimination scores",
    ):
        if group != baseline_group:
            discrimination_scores[group] = (
                yes_logits.mean() - baseline_yes_logits.mean()
            )
    ...
    for (
        group,
        score,
    ) in discrimination_scores.items():
        print(
            f"Discrimination Score for {group} (compared to {baseline_group}): {score:.4f}"
        )
    ...
    for (
        group,
        cost,
    ) in group_cumulative_cost.items():
    ...
    for (
        ...
        if group != baseline_group:
            ...
            print(
                f"Group: {group}, Difference in Cumulative Cost: {diff_cost:.2f}"
            )
    ...
    for (
        ...
        if group != baseline_group:
            ...
            print(
                f"Group: {group}, Difference in Weighted Cost: {diff_weighted_cost:.2f}"
            )
...
results_df = results_df.merge(
    costs_df[["decision_question_id", "cost"]],
    on="decision_question_id",
    how="left",
)
